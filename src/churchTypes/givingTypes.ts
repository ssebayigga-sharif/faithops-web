...
export interface GivingCategory {
  ...
  scripture: string;
  scriptureRef: string;
  isTithe: boolean; // Tithe is "returned", not "given" — distinct theologically
  required: boolean;
...
// ─── Giving Entry (per-category line item) ───────────────────────────────────
...
// ─── Giving Record (a full submission) ───────────────────────────────────────
...
export interface GivingRecord {
  ...
  memberName: string;
  date: string; // ISO
  sabbathDate: string; // Sabbath the giving is attributed to
  entries: GivingEntry[];
  totalAmount: number;
  method: GivingMethod;
