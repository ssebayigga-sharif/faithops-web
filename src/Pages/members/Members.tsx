...
import type { Member, SortField } from "@/churchTypes/memberTypes";
import { useMemberFilters } from "@/utils/useMemberFilter";
import { useMembers, useCreateMember, useDeleteMember, usePatchMember } from "@/utils/useMember";
import DeleteConfirmModal from "./DeleteMember";
import MemberModal from "./MemberModal";
import { MemberProfile } from "./memberprofile/MemberProfile";
import MemberFiltersBar from "./MemberFilters";
import { formatDate, getStatusColor, getAttendanceColor } from "@/utils/memberUtils";
...
export default function MembersPage() {
  ...
  const rows = pagedMembers.map((m) => ({
    ...
    status:         m.status,
    ministries:     m.ministries.filter((mn) => mn.active).map((mn) => mn.ministry).join(", ") || "—",
  ...
  async function handleCreateMember(member: Member) {
    try {
      ...
      const { _computed, _firebaseKey, ...payload } = member as Member & { _firebaseKey?: string };
      await createMember(payload);
      notify("success", "Member Added", `${member.firstName} ${member.lastName} saved to Firebase.`);
    } catch {
      notify("error", "Save Failed", createError ?? "Could not save member. Check your connection.");
  ...
  async function handleDeleteConfirm() {
    ...
    try {
      ...
      notify("success", "Member Removed", `${memberToDelete.firstName} ${memberToDelete.lastName} deleted.`);
  ...
  async function handleDeactivate(member: Member) {
    ...
    try {
      ...
      notify("success", "Deactivated", `${member.firstName} ${member.lastName} set to Inactive.`);
    } catch {
    ...
    <div style={{ fontFamily: "'IBM Plex Sans', sans-serif", background: "#f4f4f4", minHeight: "100vh" }}>
      ...
      <div style={{ padding: "1.5rem" }}>
        ...
        <Breadcrumb style={{ marginBottom: "0.5rem" }}>
          ...
          <BreadcrumbItem href="/dashboard/members" isCurrentPage>Members</BreadcrumbItem>
        </Breadcrumb>
        ...
        >
          <div>
            <h1 style={{ fontSize: "22px", fontWeight: 700, color: "#161616", marginBottom: 4 }}>
              Church Membership Register
            </h1>
            <p style={{ fontSize: "13px", color: "#6f6f6f" }}>
              Kampala SDA Church ·{" "}
              {isLoading ? "Loading…" : `${totalCount.toLocaleString()} total members`}
            </p>
          </div>

          <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
            <Button kind="ghost" renderIcon={Renew} size="md" onClick={() => refetch()} disabled={isLoading}>
              Refresh
            </Button>
            <Button kind="ghost" renderIcon={Upload} size="md">Import CSV</Button>
            <Button kind="secondary" renderIcon={Download} size="md">Export</Button>
            <Button kind="primary" renderIcon={Add} size="md" onClick={() => setModalOpen(true)} disabled={isLoading}>
              Add Member
            </Button>
          </div>
        </div>
        ...
        {isError && (
          <InlineNotification
            ...
            subtitle={error ?? "Check Firebase rules and your network connection."}
            lowContrast
        ...
        <div style={{ display: "flex", gap: "0.75rem", marginBottom: "1.25rem", flexWrap: "wrap" }}>
          <StatCard label="Total Members"  value={stats.total.toLocaleString()}        accent="#0f2d52" loading={isLoading} />
          <StatCard label="Active Members" value={stats.active.toLocaleString()}       accent="#198038" loading={isLoading} />
          <StatCard label="Baptized"       value={stats.baptized.toLocaleString()}     accent="#c6971a" loading={isLoading} />
          <StatCard label="New Converts"   value={stats.newConverts.toLocaleString()}  accent="#6929c4" loading={isLoading} />
        </div>
        ...
        {hasActiveFilters && (
          <p style={{ fontSize: "12.5px", color: "#6f6f6f", marginBottom: "0.5rem" }}>
            Showing {filteredCount.toLocaleString()} of {totalCount.toLocaleString()} members
          </p>
        )}
        ...
        {isLoading && (
          ...
          >
            <InlineLoading description="Loading members from Firebase…" status="active" />
          </div>
        )}
        ...
        {!isLoading && !isError && members.length === 0 && (
          ...
          >
            <p style={{ fontSize: "18px", fontWeight: 600, marginBottom: 8 }}>No members yet</p>
            <p style={{ fontSize: "13px", color: "#6f6f6f", marginBottom: "1.5rem" }}>
                ...
                <TableToolbar>
                  <TableBatchActions {...getBatchActionProps()}>
                    ...
                    <TableBatchAction
                      ...
                      onClick={() => alert(`Assign follow-up to ${selectedRows.length} member(s)`)}
                    >
                      Assign Follow-Up
                    </TableBatchAction>
                    <TableBatchAction
                      ...
                      onClick={async () => {
                        ...
                        if (!window.confirm(`Delete ${toRemove.length} member(s)?`)) return;
                    ...
                    <TableRow>
                      ...
                      {headers.map((header) => (
                        <TableHeader
                          ...
                          key={header.key}
                          onClick={() => {
                            if (["fullName","status","attendanceRate","joinedAt"].includes(header.key)) {
                        ...
                        >
                          ...
                          {row.cells.map((cell) => {
                            ...
                            if (cell.info.header === "status") {
                              return (
                                <TableCell key={cell.id}>
                                  <Tag type={getStatusColor(cell.value as any)} size="sm">
                                    {cell.value as string}
                                  </Tag>
                                </TableCell>
                              );
                            ...
                            if (cell.info.header === "attendanceRate") {
                              ...
                              const barMap: Record<string, string> = {
                                green: "#24a148", teal: "#009d9a", yellow: "#f1c21b", red: "#da1e28",
                              };
                              const barColor = barMap[getAttendanceColor(rate)];
                              return (
                                <TableCell key={cell.id}>
                                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                    <div style={{ width: 60, height: 5, background: "#f4f4f4", borderRadius: 3 }}>
                            ...
                            if (cell.info.header === "joinedAt") {
                              return (
                                <TableCell key={cell.id} style={{ fontSize: "13px", color: "#525252" }}>
                              ...
                              return (
                                ...
                                >
                                  <OverflowMenu size="sm" flipped>
                                    ...
                                    <OverflowMenuItem
                                      itemText="Deactivate"
                                      onClick={() => raw && handleDeactivate(raw)}
                                    />
                                    <OverflowMenuItem
                                      itemText="Delete Member"
                                      hasDivider
                                      isDelete
                                      onClick={() => raw && setMemberToDelete(raw)}
                                    />
                                  </OverflowMenu>
                                </TableCell>
                              );
